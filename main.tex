...
\section{Assignment}
\textbf{ Application of artificial intelligence methods to web scraping}
...
\section{Three-semester work plan (DPI--DPIII)}

\subsection*{Diploma Project I (DPI)}

\begin{itemize}
    \item Refine the problem statement, research questions, and success criteria in collaboration with the supervisor, based on the official assignment.
    \item Conduct a focused literature review on AI-based web scraping, HTML-aware language models, and small LLM optimization; curate an initial set of representative HTML pages and target schemas.
    \item Prototype the web scraping and HTML preprocessing pipeline (for example, Playwright-based crawling, DOM extraction, structure-aware linearization) and design the initial experimental setup.
\end{itemize}

\subsection*{Diploma Project II (DPII)}

\begin{itemize}
    \item Implement the full structure-aware HTML extraction pipeline and integrate selected small local LLMs together with baseline cloud LLM APIs.
    \item Construct labeled datasets using teacher-LM or rule-based annotations with manual corrections and targeted synthetic augmentation, and run the first series of experiments on extraction quality and robustness.
    \item Iterate on model adaptation strategies (parameter-efficient fine-tuning, quantization, prompt and schema design) based on pilot results, updating the experimental design as needed.
\end{itemize}

\subsection*{Diploma Project III (DPIII)}

\begin{itemize}
    \item Complete large-scale experiments on all selected tasks and domains, including ablations for key pipeline components and efficiency profiling on representative edge hardware.
    \item Analyze results in depth, answer the formulated research questions, and derive clear recommendations regarding when small on-device models are a viable alternative to cloud LLMs.
    \item Finalize the thesis manuscript, figures, and tables, prepare artefacts for reproducibility (code, configuration, datasets where possible), and create materials for the defense presentation and demonstrations.
\end{itemize}
